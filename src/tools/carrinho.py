...
@safe_tool
async def adicionar_ao_carrinho(termo: str, cep: str = settings.default_cep, quantidade: int = 1, ctx: Optional[Context] = None) -> str:
...
@safe_tool
async def ver_carrinho(ctx: Optional[Context] = None) -> str:
    sid = get_sid(ctx)
...
@safe_tool
async def remover_do_carrinho(termo: str, ctx: Optional[Context] = None) -> str:
    termo = sanitize_term(termo)
    if not termo:
...
@safe_tool
async def limpar_carrinho(ctx: Optional[Context] = None) -> str:
    sid = get_sid(ctx)
